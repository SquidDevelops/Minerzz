    ...
    public static int[] genChunk(int xO, int yO) {
        ...
        for (int height : heightMap) {
            for (int i = 0; i < height; i++)
                if (i > height - 2)
                    chunk[(i << 8) + ((pos >> 4) << 4) + (pos % 16)] = BlockType.GRASS.blockID;
                else if (i > height - 6)
                    chunk[(i << 8) + ((pos >> 4) << 4) + (pos % 16)] = BlockType.DIRT.blockID;
                else if (i == 0)
                    chunk[(i << 8) + ((pos >> 4) << 4) + (pos % 16)] = BlockType.BEDROCK.blockID;
                else
                    chunk[(i << 8) + ((pos >> 4) << 4) + (pos % 16)] = BlockType.STONE.blockID;
            pos++;
        ...
        for (int height : heightMap)
            for (int i = 0; i < height; i++)
                if (i > height - 1)
                    chunk[(i << 8) + ((pos >> 4) << 4) + (pos % 16)] = BlockType.WOOD.blockID;
        pos++;
